...
(function (window, document) {
    ...
    class PersianDatePicker {
        ...
        async selectGregorianDate(pickerState, year, month, day) {
            ...
            try {
                ...
                if (persianDate) {
                    ...
                    // Also get the server-converted DateTime for the hidden field
                    const persianDateString = `${persianDate.year}/${persianDate.month.toString().padStart(2, '0')}/${persianDate.day.toString().padStart(2, '0')}`;
                    const serverDateTime = await this.convertPersianToGregorian(persianDateString);

                    if (serverDateTime) {
                        pickerState.gregorianDate = serverDateTime;
                    }
            ...
            try {
                if (pickerState.showGregorian && pickerState.gregorianDate) {
                    ...
                    if (!isNaN(gregDate.getTime())) {
                        const formattedDate = `${gregDate.getFullYear()}/${(gregDate.getMonth() + 1).toString().padStart(2, '0')}/${gregDate.getDate().toString().padStart(2, '0')}`;
        ...
        toggleDateFormat(pickerState) {
            ...
            if (pickerState.showGregorian && pickerState.gregorianDate) {
                // Parse the gregorian date to set the view
                const gregDate = new Date(pickerState.gregorianDate);
                if (!isNaN(gregDate.getTime())) {
        ...
        async updateHiddenField(pickerState) {
            if (pickerState.selectedDate) {
                ...
                if (result) {
                    console.log(`Persian: ${persianDateString} -> Gregorian: ${result} (with server time)`);
